...
/obj/item/circuit_component/reagent_injector/proc/trigger_inject()
	...
	if(units)
		...
		add_attack_logs(bci.owner, bci.owner, "Введено с помощью [name.declent_ru(GENITIVE)] содержащим [bci.reagents.log_list()], введено [units] единиц", bci.reagents.harmless_helper() ? ATKLOG_ALMOSTALL : null)
