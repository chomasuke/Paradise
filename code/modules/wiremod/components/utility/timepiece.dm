...
/obj/item/circuit_component/timepiece/populate_ports()
	text_output = add_output_port("Формат времени", PORT_TYPE_STRING)
	num_output = add_output_port("Единица времени", PORT_TYPE_NUMBER)
...
/obj/item/circuit_component/timepiece/populate_options()
	...
	format = add_option_port("Формат времени", format_options)
	var/static/unit_options = list(
		COMP_TIMEPIECE_HOURS,
		COMP_TIMEPIECE_MINUTES,
		COMP_TIMEPIECE_SECONDS,
	)
	time_unit = add_option_port("Единица времени", unit_options)
