...
/datum/atom_hud/proc/show_to(mob/new_viewer)
	...
	if(!hud_users[their_turf.z][new_viewer])
		...
		RegisterSignal(new_viewer, COMSIG_QDELETING, .proc/unregister_atom, override = TRUE) //both hud users and hud atoms use these signals
...
/datum/atom_hud/proc/add_atom_to_hud(atom/new_hud_atom)
	...
	RegisterSignal(new_hud_atom, COMSIG_QDELETING, .proc/unregister_atom, override = TRUE) //both hud atoms and hud users use these signals
